...
//! ## Example
//!
//! Here is an example of a GFF3 header generated by `bed2gff`:
//!
//! ```plaintext
//! ##gff-version 3
//! #provider: bed2gff
...
fn write_features(i: usize, 
    ...
    if record.cds_start() < exon_end && exon_start < record.cds_end() {
        ...
        build_gff_line(record, gene_name, "CDS", start, end, frame, i as i16, file);
    }
    ...
    for line in bed {
        ...
        if let Ok(record) = record {
            ...
            if !seen_genes.contains(key?) {
                ...
                let _ = to_gff(&record, &isoforms, &mut output, true);
            } else {
                let _ = to_gff(&record, &isoforms, &mut output, false);
            };
        } else {
